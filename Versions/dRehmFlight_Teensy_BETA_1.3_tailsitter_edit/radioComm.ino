...
#include "plane.h"
  ...
  IBUS ibus(Serial5); // Changed to IBUS for consistency
  uint16_t ibuschannels[14];
  bool ibusFailSafe; // Added for iBus failsafe status
  bool ibusLostFrame; // Added for iBus lost frame status
  uint16_t radiochannels[14]; // Global declaration for radio channels
  ...
  void readRadio() {
    if (ibus.read(ibuschannels, &ibusFailSafe, &ibusLostFrame)) {
      for (int i = 0; i < 14; i++) {
        radiochannels[i] = ibuschannels[i];
      }
    }
  }
#endif
