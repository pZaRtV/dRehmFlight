...
#include <PWMServo.h> //Commanding any extra actuators, installed with teensyduino installer
  ...
  static const uint8_t num_DSM_channels = NUM_DSM_CHANNELS;
#endif

#if defined USE_IBUS_RX
  #include "src/iBus/iBus.h"
  // IBUS ibus(Serial5); // Object declared in radioComm.ino
#endif
...
float GyroErrorX = -3.90;   // default: 5.56
...
//OneShot125 ESC pin outputs: motor and prop orientation viewed from the back towards the front
const int m1Pin = 0;
const int m2Pin = 2; // default: 1 // right (CW)
const int m3Pin = 1; // default: 2 // left (CCW)
const int m4Pin = 3;
const int m5Pin = 4;
const int m6Pin = 5;
...
#if defined USE_IBUS_RX
  extern bool ibusFailSafe; // Extern declaration for iBus failsafe status
  extern bool ibusLostFrame; // Extern declaration for iBus lost frame status
#endif
...
// Radio communication:
#if defined USE_IBUS_RX
  // uint16_t ibuschannels[14]; // Reverted accidental global declaration for iBus channels
  extern IBUS ibus; // Extern declaration for iBus object
  extern uint16_t ibuschannels[14]; // Extern declaration for iBus channels
#endif
...
// Servo debug
// Serial.print("s1_scaled: "); Serial.print(s1_command_scaled);
// Serial.print(" s2_scaled: "); Serial.print(s2_command_scaled);
// Serial.print(" s1_PWM: "); Serial.print(s1_command_PWM);
// Serial.print(" s2_PWM: "); Serial.println(s2_command_PWM);
...
void getIMUdata() {
   ...
   */
  int16_t AcX,AcY,AcZ,GyX,GyY,GyZ/*,MgX,MgY,MgZ*/; //Edited for mpu6050 (no magnetometer)
 ...
 //Accelerometer
  ...
  //Magnetometer
   MagX = 0; //uT  // MgX/6.0; ( Default with 9250 ) // 0 to disable
   MagY = 0;       // MgY/6.0; ( Default with 9250 ) // 0 to disable
   MagZ = 0;       // MgZ/6.0; ( Default with 9250 ) // 0 to disable
  // //Correct the outputs with the calculated error values
...
void scaleCommands() {
  ...
  /*
   * mX_command_scaled variables from the mixer function are scaled to appropriate protocol values.
   * sX_command_scaled variables from the mixer function are scaled to 0-180 for the servo library using standard PWM.
   * mX_command_PWM are updated here which are used to command the motors. sX_command_PWM are updated 
...
void getCommands() {
   ...
   */
    ...
    // Apply PPM channel remapping (PPM_MAP_CHx) for PPM; PWM keeps 1:1
    #if defined USE_PPM_RX
      channel_1_pwm = getRadioPWM(PPM_MAP_CH1);
      channel_2_pwm = getRadioPWM(PPM_MAP_CH2);
      channel_3_pwm = getRadioPWM(PPM_MAP_CH3);
      channel_4_pwm = getRadioPWM(PPM_MAP_CH4);
      channel_5_pwm = getRadioPWM(PPM_MAP_CH5);
      channel_6_pwm = getRadioPWM(PPM_MAP_CH6);
    #else
    ...
    if (ibus.read(ibuschannels, &ibusFailSafe, &ibusLostFrame)) { // Call read with the global ibuschannels array and failsafe/lostFrame pointers
      channel_1_pwm = ibuschannels[0]; // Assuming channel 0 is throttle
      channel_2_pwm = ibuschannels[1]; // Assuming channel 1 is aileron
      channel_3_pwm = ibuschannels[2]; // Assuming channel 2 is elevator
      channel_4_pwm = ibuschannels[3]; // Assuming channel 3 is rudder
      channel_5_pwm = ibuschannels[4]; // Assuming channel 4 is gear/throttle cut
      channel_6_pwm = ibuschannels[5]; // Assuming channel 5 is aux1
    }
  #endif
...
void failSafe() {
  ...
  int check6 = 0;
    ...
    if (sbusFailSafe || sbusLostFrame) {
      check1 = 1; check2 = 1; check3 = 1; check4 = 1; check5 = 1; check6 = 1; // All channels to failsafe
    }
  #elif defined USE_IBUS_RX
    if (ibusFailSafe || ibusLostFrame) {
      check1 = 1; check2 = 1; check3 = 1; check4 = 1; check5 = 1; check6 = 1; // All channels to failsafe
    }
  #endif
